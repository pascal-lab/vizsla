...
fn write_manifest_schema(workspace_root: &Path) -> Result<()> {
    write_generated_file(
        &manifest_schema_path(workspace_root),
        &generated_manifest_schema_text()?,
    )?;
    write_generated_file(
...
fn check_manifest_schema(workspace_root: &Path) -> Result<()> {
    check_file_matches(&manifest_schema_path(workspace_root), &generated_manifest_schema_text()?)?;
    check_file_matches(
        &vscode_schema_constants_path(workspace_root),
        &generated_vscode_schema_constants_text()?,
    )?;
...
fn check_file_matches(path: &Path, expected: &str) -> Result<()> {
    ...
    if checked_in != expected {
        bail!("{} is stale; run `cargo xtask generate-manifest-schema`", path.display());
    }
