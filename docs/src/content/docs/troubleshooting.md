...
先点击 `Vizsla` 状态栏项打开状态菜单。项目配置错误会出现在菜单顶部; 也可以在菜单里选择 `Show Output`, 或直接执行 `Vizsla: Show Language Server Output`。重点看:
...
VS Code 扩展只会在包含 Verilog/SystemVerilog 文件的 workspace 缺少项目配置文件时创建默认 `vizsla.toml`:
...
这个默认项目配置文件显式写入 `sources = []`, 因此不会扫描 workspace root。需要索引项目文件或启用更准确的语义诊断时, 请写入实际的 `sources` shell glob 或 `include_dirs`, 并按需补充 `defines`, `libraries` 或 `top_modules`。如果手写项目配置时省略 `sources`, Vizsla 会进入 best-effort workspace 索引模式。我们不会自动向父目录或子目录搜索项目配置文件。
...
`vizsla.files.excludeDirs` 只接受 workspace 相对目录, 不支持 glob。文件选择请优先使用项目配置文件的 `sources` / `exclude` shell glob; 如果还要减少 VS Code 自己的 watcher 事件, 另配 VS Code 的 `files.watcherExclude`。
