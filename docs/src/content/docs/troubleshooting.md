...
这个默认项目配置文件和无项目配置文件都会索引 workspace root 来支持跳转和引用等读能力, 但不会启用跨文件 semantic diagnostics; 需要更准确的语义诊断时, 请写入实际的 `sources` shell glob 或 `include_dirs`, 并按需补充 `defines`, `libraries` 或 `top_modules`。显式写入 `sources = []` 会关闭 workspace 索引。我们不会自动向父目录或子目录搜索项目配置文件。
