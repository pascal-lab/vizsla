---
title: 当扩展无法正常启动
description: 按状态栏、输出通道和版本命令检查 Vizsla 扩展启动问题。
---

这页用于确认 Vizsla 扩展和语言服务器有没有正常接上。命令和日志入口见 [操作参考](./commands-status-logs.md)；已经出现具体错误时，转到 [故障排查](./troubleshooting.md)。
...
| 状态 | 下一步 |
| --- | --- |
| 普通 `Vizsla` 文本 | 语言服务器已经接上。悬停查看项目配置是否已加载。 |
| 旋转图标长时间不结束 | 继续看 `Vizsla Language Server` 输出通道。 |
...
执行 `Vizsla：显示服务器版本`。扩展会使用当前启动命令、工作目录和环境，把 `vizsla.server.args` 与 `--version` 组合起来执行版本查询；不会附加 `vizsla.server.additionalArgs`。如果配置了 `vizsla.server.command`，也会使用这个自定义命令。
...
如果语言服务器能启动，但需要查看更详细的内部日志，可以通过 `vizsla.server.additionalArgs` 传入：
