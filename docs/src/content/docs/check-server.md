...
这页用于确认 Vide 扩展和语言服务器有没有正常接上。命令和日志入口见 [操作参考](./commands-status-logs.md)；已经出现具体错误时，转到 [故障排查](./troubleshooting.md)。
...
执行 `Vide：显示服务器版本`。扩展会使用当前启动命令、工作目录和环境，把 `vizsla.server.args` 与 `--version` 组合起来执行版本查询；不会附加 `vizsla.server.additionalArgs`。如果配置了 `vizsla.server.command`，也会使用这个自定义命令。
