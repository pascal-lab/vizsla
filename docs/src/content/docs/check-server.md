...
最直接的判断是 VS Code 右侧状态栏里的 `Vizsla` 状态项:

- 只有 `Vizsla` 文本且无错误图标: 服务器已启动, 悬停可查看项目配置状态。
- 带旋转图标长时间不结束: 服务器启动、停止或项目配置加载卡住, 需要查看输出通道。
- 带 error 图标: 服务器启动失败或项目配置加载失败。
- 带 warning 图标: 通常表示当前 workspace 没有项目配置文件。

点击状态栏项会打开 `Vizsla Status` 菜单。可以从菜单里执行 `Show Output` 打开 `Vizsla Language Server` 输出通道。
