...
普通功能使用问题，优先回到 [功能特性](../../user-guide/features/) 的对应页面。命令、状态栏含义和输出通道名称见 [VS Code 状态栏、命令面板和日志](../../user-guide/commands-status-logs/)。
...
先确认扩展实际使用的命令。执行 `Vide：显示状态`、`Vide：显示语言服务器输出` 和 `Vide：显示服务器版本`，重点看下面几项：
