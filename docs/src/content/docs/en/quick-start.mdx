...
If this directory contains Verilog/SystemVerilog source files but has no `vizsla.toml` or legacy `vizsla_config.toml`, the extension prompts you to create a default project manifest. After you choose to create it, the extension writes a conservative `vizsla.toml` and reloads Vizsla.
