...
Follow these steps to confirm that Vizsla works in VS Code.
...
If this directory contains Verilog/SystemVerilog source files but has no `vizsla.toml` or deprecated legacy `vizsla_config.toml`, the extension prompts you to create a default project manifest. After you choose to create it, the extension writes a conservative `vizsla.toml` and reloads Vizsla.

![notification on creating manifest](../assets/quick-start/create-manifest-notification.png)

The default configuration sets `sources = []`, which means Vizsla does not scan the whole project automatically. You can leave it unchanged while trying syntax highlighting, single-file error reporting, the status bar, and commands. To tell Vizsla where your RTL files are, add the source directory to `sources`, for example:
...
If VS Code prompts you to restart Vizsla after changing server launch settings, choose `Restart`.
