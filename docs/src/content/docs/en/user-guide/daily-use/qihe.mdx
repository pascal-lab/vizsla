...
## What the Vide Integration Does
...
Vide can run Qihe on the whole project or on the current file. The command only accepts local saved RTL files: the target must be a `file:` URI, and its extension must be `.v`, `.vh`, `.sv`, `.svh`, or `.svi`.

### Analyze a Project

Project analysis needs a `vide.toml` manifest that describes source files, include directories, macro definitions, and top modules. See [Configure the First Project](../../first-project/) and [Project Configuration Reference](../../project-configuration/) for the full setup.
...
If the error is related to the Qihe command path, set `vide.qihe.command` to the absolute path of the `qihe` executable.
