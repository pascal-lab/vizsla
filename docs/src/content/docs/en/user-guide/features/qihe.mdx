...
Qihe and Vide are complementary. Vide focuses on fast, immediate static-analysis feedback during hardware development, so it is not intended for deeper and more computationally expensive tasks such as hardware vulnerability mining. Qihe focuses on deep static analysis for non-real-time workflows.

Vide can call the Qihe command-line tool using information from `vide.toml`, run Qihe analysis for the whole project, and show Qihe diagnostics in the editor. Together, Vide and Qihe provide both immediate feedback and deeper analysis results inside the same VS Code workflow.
...
Vide currently supports running Qihe for the whole project or the current file.

### Analyze a Project

Project analysis needs `vide.toml` to describe source files, include directories, macro definitions, and top modules. See [Configure the First Project](../../first-project/) and [Project Configuration Reference](../../project-configuration/) for the full setup.
...
This code intentionally contains a divide-by-zero expression. After Qihe runs, the editor shows a diagnostic from `ZeroDivisionAnalysis`.
...
If the workspace has no `vide.toml`, `Vide: Run Qihe Analysis` sends the currently open RTL file to Qihe.
...
For project analysis, Vide reads `vide.toml` and builds the Qihe compile input. With the default settings, the commands look like this:
...
See [Qihe settings](../../vscode-settings/#qihe) for the full reference.

Vide then parses Qihe diagnostics and shows them in the editor. Command execution details are available in VS Code's Output panel: choose the `Vide Qihe` output channel.

If you do not want Vide to generate `--mode sv`, `--top`, `-I`, and `-D` from `vide.toml`, set `vide.qihe.autoConfigureArgsFromManifest` to `false` in VS Code Settings. After that, input files still come from the current project or the current file, and the commands become:
...
4. Whether Qihe stdout or stderr contains an explicit compile or run error.
...
If the problem comes from Qihe's own compile or run process, file an issue with the [Qihe project](https://qihe.pascal-lab.net/).
