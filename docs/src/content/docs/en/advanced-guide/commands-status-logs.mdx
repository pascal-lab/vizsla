...
| <Codicon name="warning" label="Warning icon" /> `Vide` | Usually means the current workspace has no project manifest. |
| <Codicon name="error" label="Error icon" /> `Vide` | Server startup or project configuration loading failed. |

Click the `Vide` status item or run `Vide: Show Status` to open the status menu. Menu items vary with project state; common actions include opening or creating the project manifest, profiling diagnostics, reloading the project, restarting the language server, and showing output.
