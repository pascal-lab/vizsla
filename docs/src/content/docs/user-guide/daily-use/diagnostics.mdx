...
默认 `vizsla.diagnostics.update` 是 `onSave`，保存文件时刷新诊断。大型工程建议保持这个默认值，避免输入时反复触发较重分析。
...
先保存当前文件。仍然没有更新时，执行 `Vide：重新加载项目配置`；如果项目配置刚改过，这一步会重新读取 `vizsla.toml`。再不行，执行 `Vide：重启语言服务器`，并点击状态栏 `Vide` 看是否有项目配置错误。
...
灯泡里没有这些操作时，可以手动编辑 `vizsla.diagnostics.slang.rules`。规则写法见 [Diagnostics 设置参考](../../../advanced-guide/vscode-settings/#diagnostics)。
