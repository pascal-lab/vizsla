---
title: Vizsla 如何分析无配置项目
description: 解释 Vizsla 在缺少完整项目配置时会读取哪些文件、哪些功能会受限。
---
...
- 尽力索引：Vizsla 在没有完整项目配置时，尽量读取工作区里的 Verilog/SystemVerilog 文件，让跳转、引用、悬停、补全等阅读功能先可用。它适合“先读代码”，不适合替代真实工程配置。
- 项目分析：Vizsla 根据项目配置文件里的 `sources`、`include_dirs`、`defines`、`libraries` 和 `top_modules` 建立的工程视图。推荐文件名是 `vizsla.toml`；旧版 `vizsla_config.toml` 仍兼容但已弃用。跨文件诊断和 Qihe 工程分析依赖它。
...
在尽力索引里，如果多个同名 module 都可见，Vizsla 会只为编辑器阅读功能做一次就近推测：优先同文件，再优先共同目录最深的候选，最后优先同一扫描根。只有出现唯一最优候选时才会采用；如果打平，结果仍然保持歧义。
...
Qihe 自动工程分析使用与 Vizsla 项目模型相同的配置文件发现结果。工作目录下存在推荐的 `vizsla.toml` 时会优先读取它；只有旧版 `vizsla_config.toml` 时仍会兼容进入工程分析，但这个文件名已弃用，建议迁移为 `vizsla.toml`。

存在项目配置文件时，Qihe 会使用项目分析得到的编译计划。只有计划里存在实际源文件时，Vizsla 才会传入工程文件、`--top`、`-I` 和 `-D` 参数。
