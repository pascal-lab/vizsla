...
Vizsla 的项目配置文件名优先使用 `vizsla.toml`。旧文件名 `vizsla_config.toml` 仍然兼容, 但当两个文件同时存在时会读取 `vizsla.toml`。项目配置文件必须放在 workspace root 下, 或者被显式作为工程路径传入。字段名必须使用当前支持的名字, 旧字段或未知字段会被拒绝。
...
`sources` 和 `exclude` 使用 workspace-relative shell glob 语义, 统一用 `/` 作为路径分隔符。`include_dirs` 和 `libraries` 仍然是相对于项目配置文件所在目录解析的路径。

VS Code 在包含 Verilog/SystemVerilog 文件的 workspace 缺少项目配置文件时会生成默认 `vizsla.toml`:
...
这个默认项目配置文件显式写入 `sources = []`, 因此不会扫描 workspace 下的源文件, 也不会建立编译 profile 或运行跨文件 semantic diagnostics。如果手写项目配置时省略 `sources`, Vizsla 会以 best-effort 方式索引 workspace 下的 Verilog/SystemVerilog 文件, 用于跳转、引用等读能力, 但不会建立编译 profile 或运行跨文件 semantic diagnostics。Best-effort 索引遇到重名 module 时只会在唯一最近候选存在时按路径距离推测; 能唯一推测时不报告 diagnostic, 无法唯一推测时会以 `ambiguous-module-instantiation` information 标出。配置化工程不会把目录当作隐式命名空间。需要更准确的 semantic diagnostics 时, 请写入符合工程结构的 `sources` shell glob 或 `include_dirs`, 并按需补充 `defines`, `libraries` 或 `top_modules`。

Vizsla 只读取项目配置文件来构建工程模型, 并在项目配置文件变更后刷新工程信息。文件开头的 schema directive 是 TOML 注释, 不会影响 Vizsla 读取项目配置文件。schema URL 带有版本段; 后续项目配置格式变化时, 新版 Vizsla 可以生成指向新版 schema 的 directive, 已有工程仍保留原来的 schema。
...
- 如果库目录下有 `vizsla.toml`, 我们会按这个项目配置文件加载库; 否则回退到 `vizsla_config.toml`。
...
- 推荐项目配置文件名是 `vizsla.toml`; `vizsla_config.toml` 仅作为兼容旧工程的 fallback。
