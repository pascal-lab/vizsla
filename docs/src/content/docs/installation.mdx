---
title: 安装
description: 通过 VS Code Marketplace、GitHub 发布包或自定义服务器安装 Vizsla。
---
...
<ThinLinkCard
  ...
  action="打开"
>
  安装稳定版 Vizsla，扩展 ID：<code>vizsla.vizsla-lsp</code>
</ThinLinkCard>

也可以在 VS Code 扩展面板搜索显示名 `Vizsla`，或搜索扩展 ID `vizsla.vizsla-lsp`。安装后打开 RTL 工程目录，扩展会自动使用随扩展打包的 `vizsla` 语言服务器。
