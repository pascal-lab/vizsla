...
bool isKnownSyntaxKind(SyntaxKind kind) {
    return kind != SyntaxKind::Unknown;
}

bool isPossibleMemberAllowed(TokenKind tokenKind, bool (*allowed)(SyntaxKind)) {
    for (auto memberKind : SyntaxKind_traits::values) {
        if (allowed(memberKind) && SyntaxFacts::isPossibleMemberKind(tokenKind, memberKind))
            return true;
    }
    return false;
}

bool isKeywordCandidateForContext(SyntaxKeywordContext context, TokenKind kind) {
    switch (context) {
        case SyntaxKeywordContext::CompilationUnitMember:
            return isPossibleMemberAllowed(kind, SyntaxFacts::isAllowedInCompilationUnit);
        case SyntaxKeywordContext::LibraryMapMember:
            return isKnownSyntaxKind(SyntaxFacts::getLibraryMapMemberKind(kind));
        case SyntaxKeywordContext::ModuleHeaderItem:
        case SyntaxKeywordContext::AnsiPortItem:
            return SyntaxFacts::isPossibleAnsiPort(kind);
        case SyntaxKeywordContext::ModuleMember:
            return isPossibleMemberAllowed(kind, SyntaxFacts::isAllowedInModule);
        case SyntaxKeywordContext::GenerateMember:
            return isPossibleMemberAllowed(kind, SyntaxFacts::isAllowedInGenerate);
        case SyntaxKeywordContext::SpecifyItem:
            return isKnownSyntaxKind(SyntaxFacts::getSpecifyItemKind(kind));
        case SyntaxKeywordContext::ConfigHeaderItem:
            return isKnownSyntaxKind(SyntaxFacts::getConfigHeaderItemKind(kind));
        case SyntaxKeywordContext::ConfigRule:
            return isKnownSyntaxKind(SyntaxFacts::getConfigRuleKind(kind));
        case SyntaxKeywordContext::BlockItem:
            return isKnownSyntaxKind(SyntaxFacts::getBlockItemDeclarationKind(kind)) ||
                   SyntaxFacts::isPossibleStatement(kind);
        case SyntaxKeywordContext::Statement:
            return SyntaxFacts::isPossibleStatement(kind);
        case SyntaxKeywordContext::ParameterPortListItem:
            return SyntaxFacts::isPossibleParameter(kind);
        case SyntaxKeywordContext::FunctionPortItem:
            return SyntaxFacts::isPossibleFunctionPort(kind);
        case SyntaxKeywordContext::GateType:
            return SyntaxFacts::isGateType(kind);
        default:
            return false;
    }
}
...
bool SyntaxFacts::isPossibleMemberKind(TokenKind tokenKind, SyntaxKind memberKind) {
    if (getModuleDeclarationKind(tokenKind) == memberKind)
        return true;

    if (getProceduralBlockKind(tokenKind) == memberKind)
        return true;

    if (isGateType(tokenKind) && memberKind == SyntaxKind::PrimitiveInstantiation)
        return true;

    if (isPortDirection(tokenKind) && memberKind == SyntaxKind::PortDeclaration)
        return true;

    if (tokenKind == TokenKind::ConstKeyword && memberKind == SyntaxKind::PortDeclaration)
        return true;

    if (isNetType(tokenKind) && memberKind == SyntaxKind::NetDeclaration)
        return true;

    if (getBlockItemDeclarationKind(tokenKind) == memberKind)
        return true;

    switch (tokenKind) {
        case TokenKind::GenerateKeyword:
            return memberKind == SyntaxKind::GenerateRegion;
        case TokenKind::BeginKeyword:
            return memberKind == SyntaxKind::GenerateBlock;
        case TokenKind::TimeUnitKeyword:
        case TokenKind::TimePrecisionKeyword:
            return memberKind == SyntaxKind::TimeUnitsDeclaration;
        case TokenKind::InterfaceKeyword:
            return memberKind == SyntaxKind::ClassDeclaration;
        case TokenKind::ModPortKeyword:
            return memberKind == SyntaxKind::ModportDeclaration;
        case TokenKind::BindKeyword:
            return memberKind == SyntaxKind::BindDirective;
        case TokenKind::SpecParamKeyword:
            return memberKind == SyntaxKind::SpecparamDeclaration;
        case TokenKind::AliasKeyword:
            return memberKind == SyntaxKind::NetAlias;
        case TokenKind::SpecifyKeyword:
            return memberKind == SyntaxKind::SpecifyBlock;
        case TokenKind::AssertKeyword:
        case TokenKind::AssumeKeyword:
        case TokenKind::CoverKeyword:
            return memberKind == SyntaxKind::ImmediateAssertionMember ||
                   memberKind == SyntaxKind::ConcurrentAssertionMember;
        case TokenKind::RestrictKeyword:
            return memberKind == SyntaxKind::ConcurrentAssertionMember;
        case TokenKind::AssignKeyword:
            return memberKind == SyntaxKind::ContinuousAssign;
        case TokenKind::ForKeyword:
            return memberKind == SyntaxKind::LoopGenerate;
        case TokenKind::IfKeyword:
            return memberKind == SyntaxKind::IfGenerate;
        case TokenKind::CaseKeyword:
            return memberKind == SyntaxKind::CaseGenerate;
        case TokenKind::GenVarKeyword:
            return memberKind == SyntaxKind::GenvarDeclaration;
        case TokenKind::TaskKeyword:
            return memberKind == SyntaxKind::TaskDeclaration;
        case TokenKind::FunctionKeyword:
            return memberKind == SyntaxKind::FunctionDeclaration;
        case TokenKind::CoverGroupKeyword:
            return memberKind == SyntaxKind::CovergroupDeclaration;
        case TokenKind::ClassKeyword:
        case TokenKind::VirtualKeyword:
            return memberKind == SyntaxKind::ClassDeclaration;
        case TokenKind::DefParamKeyword:
            return memberKind == SyntaxKind::DefParam;
        case TokenKind::ImportKeyword:
            return memberKind == SyntaxKind::PackageImportDeclaration ||
                   memberKind == SyntaxKind::DPIImport;
        case TokenKind::ExportKeyword:
            return memberKind == SyntaxKind::PackageExportDeclaration ||
                   memberKind == SyntaxKind::PackageExportAllDeclaration ||
                   memberKind == SyntaxKind::DPIExport;
        case TokenKind::Semicolon:
            return memberKind == SyntaxKind::EmptyMember;
        case TokenKind::PropertyKeyword:
            return memberKind == SyntaxKind::PropertyDeclaration;
        case TokenKind::SequenceKeyword:
            return memberKind == SyntaxKind::SequenceDeclaration;
        case TokenKind::CheckerKeyword:
            return memberKind == SyntaxKind::CheckerDeclaration;
        case TokenKind::GlobalKeyword:
            return memberKind == SyntaxKind::DefaultClockingReference;
        case TokenKind::DefaultKeyword:
            return memberKind == SyntaxKind::DefaultClockingReference ||
                   memberKind == SyntaxKind::DefaultDisableDeclaration;
        case TokenKind::ClockingKeyword:
            return memberKind == SyntaxKind::ClockingDeclaration;
        case TokenKind::ConstraintKeyword:
            return memberKind == SyntaxKind::ConstraintDeclaration;
        case TokenKind::SystemIdentifier:
            return memberKind == SyntaxKind::ElabSystemTask;
        case TokenKind::PrimitiveKeyword:
            return memberKind == SyntaxKind::UdpDeclaration;
        case TokenKind::RandKeyword:
            return memberKind == SyntaxKind::CheckerDataDeclaration;
        case TokenKind::ExternKeyword:
            return memberKind == SyntaxKind::ExternModuleDecl ||
                   memberKind == SyntaxKind::ExternUdpDecl ||
                   memberKind == SyntaxKind::ExternInterfaceMethod;
        case TokenKind::ConfigKeyword:
            return memberKind == SyntaxKind::ConfigDeclaration;
        default:
            return false;
    }
}

SyntaxKind SyntaxFacts::getBlockItemDeclarationKind(TokenKind kind) {
    switch (kind) {
        ...
        default:
            if (getIntegerType(kind) != SyntaxKind::Unknown ||
                getKeywordType(kind) != SyntaxKind::Unknown || isPossibleDataType(kind)) {
                return SyntaxKind::DataDeclaration;
            }
            return SyntaxKind::Unknown;
    }
...
SyntaxKind SyntaxFacts::getLibraryMapMemberKind(TokenKind kind) {
    switch (kind) {
...
SyntaxKind SyntaxFacts::getSpecifyItemKind(TokenKind kind) {
    switch (kind) {
        ...
        case TokenKind::IfNoneKeyword:
            return SyntaxKind::IfNonePathDeclaration;
        case TokenKind::IfKeyword:
            return SyntaxKind::ConditionalPathDeclaration;
        case TokenKind::OpenParenthesis:
            return SyntaxKind::PathDeclaration;
        case TokenKind::SystemIdentifier:
            return SyntaxKind::SystemTimingCheck;
        default:
            return SyntaxKind::Unknown;
    }
}

SyntaxKind SyntaxFacts::getConfigHeaderItemKind(TokenKind kind) {
    switch (kind) {
        case TokenKind::DesignKeyword:
            return SyntaxKind::ConfigDeclaration;
        case TokenKind::LocalParamKeyword:
            return SyntaxKind::ParameterDeclarationStatement;
        default:
            return SyntaxKind::Unknown;
...
SyntaxKind SyntaxFacts::getConfigRuleKind(TokenKind kind) {
    switch (kind) {
        case TokenKind::DefaultKeyword:
            return SyntaxKind::DefaultConfigRule;
        case TokenKind::CellKeyword:
            return SyntaxKind::CellConfigRule;
        case TokenKind::InstanceKeyword:
            return SyntaxKind::InstanceConfigRule;
        default:
            return SyntaxKind::Unknown;
    }
}

std::vector<std::string> SyntaxFacts::getKeywordCandidates(parsing::KeywordVersion version,
                                                           SyntaxKeywordContext context) {
    std::vector<std::string> keywords;
    auto* table = parsing::LexerFacts::getKeywordTable(version);
    if (!table)
        return keywords;

    keywords.reserve(table->size());
    for (const auto& [text, kind] : *table) {
        if (isKeywordCandidateForContext(context, kind))
