...
std::span<SyntaxNode*> Parser::parseBlockItems(TokenKind endKind, Token& end, bool inConstructor) {
    ...
    while (true) {
        kind = peek().kind;
        recordExpectedSyntax(
            diag::ExpectedStatement, TokenKind::Unknown,
            sawStatement ? SyntaxKeywordContext::Statement : SyntaxKeywordContext::BlockItem);
        if (isEndKeyword(kind) || kind == endKind || kind == TokenKind::EndOfFile)
            break;
