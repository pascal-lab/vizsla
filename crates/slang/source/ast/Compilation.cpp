...
std::vector<std::string> Compilation::getSystemFunctionNames() const {
    std::vector<std::string> names;
    for (const auto& [name, subroutine] : subroutineMap) {
        if (!name.empty() && name.front() == '$' && subroutine->kind == SubroutineKind::Function)
...
std::vector<std::string> Compilation::getSystemTaskNames() const {
    std::vector<std::string> names;
    for (const auto& [name, subroutine] : subroutineMap) {
        if (!name.empty() && name.front() == '$' && subroutine->kind == SubroutineKind::Task)
            names.emplace_back(name.data(), name.size());
    }
