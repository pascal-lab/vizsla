...
/// Grammar contexts that can be queried for keyword tokens that begin a syntax item.
enum class SyntaxKeywordContext : uint8_t {
...
public:
    ...
    /// @return true if the given token can begin a member with the given syntax kind.
    static bool isPossibleMemberKind(TokenKind tokenKind, SyntaxKind memberKind);

    /// @return the block item declaration kind that can begin with the given token.
    static SyntaxKind getBlockItemDeclarationKind(TokenKind kind);

    /// @return the library map member kind that can begin with the given token.
    static SyntaxKind getLibraryMapMemberKind(TokenKind kind);

    /// @return the specify item kind that can begin with the given token.
    static SyntaxKind getSpecifyItemKind(TokenKind kind);

    /// @return the config declaration header item kind that can begin with the given token.
    static SyntaxKind getConfigHeaderItemKind(TokenKind kind);

    /// @return the config rule kind that can begin with the given token.
    static SyntaxKind getConfigRuleKind(TokenKind kind);

    /// @return the keywords from @a version that can begin syntax in @a context.
    static std::vector<std::string> getKeywordCandidates(parsing::KeywordVersion version,
