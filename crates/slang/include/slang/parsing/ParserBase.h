...
protected:
    ParserBase(Preprocessor& preprocessor, const Bag& options);
    ...
    void recordExpectedSyntax(
        DiagCode code, TokenKind tokenKind = TokenKind::Unknown,
        std::optional<syntax::SyntaxKeywordContext> keywordContext = std::nullopt);
    std::vector<ExpectedSyntax> takeExpectedSyntax();
...
private:
    ...
    ExpectedSyntaxOptions expectedSyntaxOptions;
    std::vector<ExpectedSyntax> expectedSyntax;
};
