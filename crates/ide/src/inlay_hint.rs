use hir::{
    ...
    scope::{AnsiPortEntry, ModuleEntry, ModuleScope, NonAnsiPortEntry, UnitEntry},
                    ...
                    match &target_module.ports {
                        ...
                        Ports::Ansi(_) => {
                            let (port_decl_id, decl_id) =
                                ansi_port_decl_id_for_conn(db, target_module_id, conn, idx)?;
...
fn collect_connection_hint(
    ...
    match conn {
        ...
        PortConn::Named(port_name, expr) => {
            ...
            collector.collect_hint(conn_src, target_src, Some(position), label, None);
        }
        PortConn::Wildcard => {}
    }
...
fn ansi_port_decl_id_for_conn(
    ...
    match conn {
        ...
        PortConn::Named(Some(name), _) => {
            ...
            let ModuleEntry::AnsiPortEntry(AnsiPortEntry(decl_id)) = module_scope.get(name)? else {
                return None;
            };
